...
import jBittorrentAPI.LogManager;
...
//    }
    ...
    public static byte[] readBytesFromFile(File file) {
        ...
        try {
            ...
            if (!file.canRead()) {
                System.err.println(
                        "Error: [TorrentFileHandler.java] Cannot decode from \""
                        + file.getName()
                        +
            ...
            if (file_size_long > Integer.MAX_VALUE) {
                System.err.println(
                        "Error: [TorrentFileHandler.java] The file \"" +
                        file.getName()
                        + "\" is too large to be decode by this class.");
                return null;
            }
            ...
            if (file_offset < file_bytes.length) {
                throw new IOException("Could not completely decode file \""
                                      + file.getName() + "\".");
            }
    ...
    public static String readUserInput(String question) {
        ...
        // Setup the I/O buffered stream, to decode user input from the command line
