...
public class DownloadManager implements DTListener, PeerUpdateListener,
     ...
     * @param clientID byte[]
     */
    public DownloadManager(Torrent torrent, final byte[] clientID) {
        ...
        this.nbPieces = torrent.getPieces().size();
        this.pieceList = new Piece[this.nbPieces];
        this.nbOfFiles = this.torrent.getInfo().getFiles().size();
        ...
        this.length = this.torrent.getInfo().getTotalFilesLength();
        this.left = this.length;

        this.checkTempFiles();

        /**
         * Construct all the pieces with the correct length and hash getParamName
         */
        int file = 0;
        int fileoffset = 0;
        for (int i = 0; i < this.nbPieces; i++) {
            ...
            do {
                ...
                if (fileoffset + this.torrent.getPieceLength() - pieceoffset >=
                    (Integer) (torrent.getFileLengths().get(file)) &&
                    i != this.nbPieces - 1) {
                    pieceoffset += ((Integer) (torrent.getFileLengths().get(file))).
                            intValue() - fileoffset;
                    file++;
                    fileoffset = 0;
                    if (pieceoffset == this.torrent.getPieceLength())
                        break;
                } else {
                    fileoffset += this.torrent.getPieceLength() - pieceoffset;
                    break;
                }
            } while (true);
            pieceList[i] = new Piece(i,
                                     ...
                                     intValue(),
                                     16384, (byte[]) torrent.
                                     getPieceHashValuesAsBinary().get(i), tm);
    ...
    public synchronized int checkTempFiles() {
        ...
        if (this.nbOfFiles > 1)
            saveas += this.torrent.getSaveAs() + "/";
        new File(saveas).mkdirs();
        for (int i = 0; i < this.nbOfFiles; i++) {
            File temp = new File(saveas + ((String) (this.torrent.getFilePaths().get(i))));
            try {
                this.output_files[i] = new RandomAccessFile(temp, "rw");
                this.output_files[i].setLength((Integer) this.torrent.getFileLengths().get(
                        i));
    ...
    public synchronized void savePiece(int piece) {
        //int remaining = this.pieceList[piece].getFileLengths();
        ...
        for (Iterator it = this.pieceList[piece].getFileAndOffset().keySet().
            ...
            try {
                ...
                int remaining = ((Integer)this.torrent.getFileLengths().get(file.intValue())).
                                intValue()
                                -
                ...
                for (int i = 0; i < this.nbOfFiles; i++) {
                    try {
                        ...
                        FileOutputStream fos = new FileOutputStream(saveAs +
                                ((String) (this.torrent.getFilePaths().get(i))));
                        fos.write(Utils.subArray(data, offset,
                                ((Integer) (this.torrent.getFileLengths()
                                        .get(i))).intValue()));
                        fos.flush();
                        fos.close();
                        offset += ((Integer) (this.torrent.getFileLengths().get(i))).
                                intValue();
                    } catch (IOException ioe) {
                        ioe.printStackTrace();
    ...
    public synchronized void pieceCompleted(String peerID, int i,
        ...
        synchronized (this.isComplete) {
            if (complete && !this.isPieceComplete(i)) {
                pu.updateParameters(this.torrent.getPieceLength(), 0, "");
        ...
        if (this.isPieceComplete(piece)) {
            ...
            if (dt != null) {
                dt.ms.addMessageToQueue(new Message_PP(
                        PeerProtocol.PIECE,
                        Utils.concat(Utils.intTo4ByteArray(piece),
                                Utils.concat(Utils.intTo4ByteArray(begin),
    ...
    public synchronized void connect(Peer p) {
        DownloadTask dt = new DownloadTask(p,
                                           this.torrent.getInfoHashAsBinary(),
                                           this.clientID, true,
                                           this.getBitField());
