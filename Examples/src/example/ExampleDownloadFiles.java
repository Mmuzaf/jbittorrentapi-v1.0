 ...
 * protocol with a given .torrentMetaInfo file
 */
class ExampleDownloadFiles {
    ...
    public ExampleDownloadFiles(String[] args){
        ...
        try {
            if(args.length < 1){
                System.err.println(
                        "Incorrect use, please provide the path of the torrentMetaInfo file...\r\n" +
                        "\r\nCorrect use of ExampleDownloadFiles:\r\n"+
                        "ExampleDownloadFiles torrentPath");

                System.exit(1);
            }
            Torrent t = new TorrentBuilder(args[0]).getTorrent();
            if(args.length > 1)
            ...
            if (t != null) {
                ...
                System.err.println(
                        "Provided file is not a valid torrentMetaInfo file");
                System.err.flush();
                System.exit(1);
            }
        } catch (Exception e) {

            System.out.println("Error while processing torrentMetaInfo file. Please restart the client");
            //e.printStackTrace();
            System.exit(1);
        }
